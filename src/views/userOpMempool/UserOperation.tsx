...
function RecentUserOps(props: any) {
    ...
    useEffect(() => {
        const getAndStoreMempoolData = async (hash: string) => {
            if (hash) {
                let pollCondition = true;
                setTableLoading(false);
                while (pollCondition) {
                    const mempoolData = await getMempoolData(hash);
                    console.log('refresh');
                    setMempoolData(mempoolData);
                    await sleep(2000);
                    if ('reciept' in mempoolData && 'blockNumber' in mempoolData.reciept) pollCondition = false;
                }
            }
        };
        getAndStoreMempoolData(hash);
    }, [hash]);
    ...
    return (
        <div className="">
            ...
            <div>
                ...
                >
                    {'userOpHash' in mempoolData ? (
                        <>
                            <HeaderSection item={mempoolData} network={'mainnet'} loading={tableLoading} />
                            <TransactionDetails
