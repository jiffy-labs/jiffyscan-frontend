...
export default function TransactionDetails({
    ...
    const renderDisplayFee = () => {
        const fee = (value: any, index: number) =>
          ...
          return <>{listFee.map((value: any, index: number) => { return fee(value, index) })}</>
    }
                            ...
                            <div>
                                <section className="">
                                    <div className="container px-0 rounded">
                                        <InfoSection icon='sender' title="Sender" content={
                                            <>
                                        ...
                                        {((targets && targets.length > 0) || item?.target) && (
                                            <InfoSection icon='target' title="Target" content={
                                                 targets.length > 0 ? renderTarget(targets, true)
                                                    : (renderTarget(item.target, false))}
                                            />)}
                                        <InfoSection icon='blockTime' title="Block Time" content={
                                        ...
                                        <InfoSection icon='status' title="Status" content={
                                            <div className="flex items-center gap-[10px]">
                                                <Tooltip
                                                    arrow={true}
                                                    placement="top"
                                        ...
                                        {type && (
                                            <InfoSection icon='transactionType' title="Value" content={
                                            <div className="flex items-center gap-[10px]">
                                                <div className="flex items-center gap-[10px]">
                                                    {type}
                                                </div>
                                        ...
                                        {(values?.length > 0) || (item?.value && (
                                          <InfoSection icon='value' title="Value" content={
                                            <span className="text-dark-600 md:text-[14px] text-[16px] break-all leading-5">
                                                {renderDisplayFee()}
                                                {/*<RenderDisplayFee  items={ values?.length > 0 ? values : [item.value]} item={item} type={type} />*/}
                                            </span>
                                        } />))}
                                        <InfoSection icon='gasFee' title="Gas Fee" content={
                                            <div className="flex items-center gap-[10px]">
                                                <DisplayFee item={item?.actualGasCost!} network={item?.network} />
                                            </div>
                                        } />
                                        <InfoSection icon='gasUsed' title="Gas Used" content={
                                            <div className="flex items-center gap-[10px]">
                                                {item?.actualGasUsed}
                                            </div>
                                        } />
                                        <InfoSection icon='paymaster' title="Paymaster" content={
                                            <>
                                                <div className="flex items-center gap-[10px]">
                                                    <RenderTextCopyLink text={item?.sender}  network ={item?.network} type ='paymaster' active={item?.paymaster === '0x0000000000000000000000000000000000000000'}/>
                                                </div>
                                                <PowerButton item ={item?.paymaster?.toLowerCase()} addressMapping={addressMapping}/>
                                            </>
                                        } />
                                        <InfoSection icon='beneficiary' title="Beneficiary" content={
                                            <>
                                        ...
                                        <InfoSection icon='transactionHash' title="Transaction Hash" content =
                                            { <RenderTextCopyLink text={item?.transactionHash}  network ={item?.network} type ='bundler'/>}
                                         />
                                        <InfoSection icon='block' title="Block" content ={
                                            <RenderTextCopyLink text={item?.blockNumber}  network ={item?.network} type ='block'/>}
                                        />
                                        {item?.erc20Transfers?.length > 0 &&
                                            <InfoSection icon='erc20' title="ERC-20 Tokens Transferred" content = {
                                                <div className="flex flex-col gap-[10px] w-full">
                                                    {item.erc20Transfers
                                                        .slice(0)
                                                        .reverse()
                                                        .map(({to, from, value, name, decimals, contractAddress, symbol, }: UserOp['erc20Transfers'],
                                                              index: number,
                                                            ) => (
                                                                <ERC20Transfers
                                        ...
                                        {item?.erc721Transfers?.length > 0 && (
                                            <InfoSection icon='erc721' title="ERC-20 Tokens Transferred" content = {
                                                <div className="flex flex-col gap-[10px] w-full">
                                                    {item.erc721Transfers
                                                        .slice(0)
                                                        .reverse()
                                                        .map(
                                                            ({ to, from, tokenId, name, decimals, contractAddress, symbol,
