    ...
    <footer className="bg-dark-600 py-12 text-white">
      <div className="container flex flex-col gap-12">
            ...
            <div key={id} className="place-self-center self-start">
              ...
              <div className="flex flex-col gap-5">
                {lists.map(([name, url], index) => (
                  <Link className="flex items-center gap-2 font-bold text-white" key={index} href={url}>
                    <span>{name}</span>
                    <img src="/images/icon-container (17).svg" alt="" />
                  </Link>
                ))}
              </div>
            </div>
        ...
        <div className="flex gap-10 justify-between ">
          ...
          <p className="text-sm text-[#CFD8DC] flex items-center gap-2">
