...
function Menu(props: { name: string, dropdown: string[][] | undefined, id: string, url: string }) {
    ...
    return (
        <div className="relative" id={props.id}>
            ...
            {isOpen && (
                <div className="absolute top-full left-0 bg-white rounded-lg py-2 px-4">
                    <ul>
                        {props.dropdown?.map(menuItem => <li><a className="text-gray-700 block px-4 py-2 text-sm" href={menuItem[1]}>{menuItem[0]}</a></li>)}
                    </ul>
                </div>
            )}
        </div>
